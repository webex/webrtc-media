/**
 * mocking RTCPeerConnection where we have createoffer function which creates offer to establish connection between peers
 * created offer returns object which has two properties type and sdp
 * mockOne has valid sdp will allow developer to have  media codec available in browser
 * mockTwo has invalid sdp will not allow offer to be created and will log error
 */
...
class RTCPeerConnectionMock {
  ...
  async createOffer() {
    ...
    switch (RTCPeerConnectionMock.fakeIdentifier) {
      ...
      case 'three':
        // returning a promise that is pending in order to check if media codec waits for codec to be loaded in the browser
